 ...
 */
package com.alium.ALMPlayer.Services;
...
import com.alium.ALMPlayer.AsyncTasks.AsyncGetSongStreamURLTask;
import com.alium.ALMPlayer.BroadcastReceivers.HeadsetButtonsReceiver;
import com.alium.ALMPlayer.BroadcastReceivers.HeadsetPlugBroadcastReceiver;
import com.alium.ALMPlayer.DBHelpers.DBAccessHelper;
import com.alium.ALMPlayer.Helpers.AudioManagerHelper;
import com.alium.ALMPlayer.Helpers.EqualizerHelper;
import com.alium.ALMPlayer.Helpers.SongHelper;
import com.alium.ALMPlayer.PlaybackKickstarter.PlaybackKickstarter.BuildCursorListener;
import com.alium.ALMPlayer.R;
import com.alium.ALMPlayer.RemoteControlClient.RemoteControlClientCompat;
import com.alium.ALMPlayer.RemoteControlClient.RemoteControlHelper;
import com.alium.ALMPlayer.Scrobbling.ScrobbleDroidHelper;
import com.alium.ALMPlayer.Scrobbling.SimpleLastFMHelper;
import com.alium.ALMPlayer.Utils.Common;
import com.alium.ALMPlayer.WidgetProviders.AlbumArtWidgetProvider;
import com.alium.ALMPlayer.WidgetProviders.BlurredWidgetProvider;
import com.alium.ALMPlayer.WidgetProviders.LargeWidgetProvider;
import com.alium.ALMPlayer.WidgetProviders.SmallWidgetProvider;
...
public class AudioPlaybackService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		Log.d("SERVICE: ", "I DON START WORK");
	 ...
	 */
	private Notification buildMNotification(SongHelper songHelper) {
		mNotificationBuilder = new NotificationCompat.Builder(mContext);
		mNotificationBuilder.setOngoing(true);
		mNotificationBuilder.setAutoCancel(false);
		mNotificationBuilder.setSmallIcon(R.drawable.notif_icon);
	...
	private Notification buildKitKatNotification(SongHelper songHelper) {
		mNotificationBuilder = new NotificationCompat.Builder(mContext);
		mNotificationBuilder.setOngoing(true);
		mNotificationBuilder.setAutoCancel(false);
		mNotificationBuilder.setSmallIcon(R.drawable.notif_icon);
	...
	/*public boolean openFile(final String path) {
		if (D) Log.d("AUDIO PLAYING", "openFile: path = " + path);
		synchronized (this) {
			if (path == null) {
				return false;
			}
	...
	private Notification buildLollipopNotification(SongHelper songHelper) {
		mNotificationBuilder = new NotificationCompat.Builder(mContext);
		mNotificationBuilder.setOngoing(true);
		mNotificationBuilder.setAutoCancel(false);
		mNotificationBuilder.setSmallIcon(R.drawable.notif_icon);
	...
	private Notification buildNotification(SongHelper songHelper) {
		if (Build.VERSION.SDK_INT == Build.VERSION_CODES.JELLY_BEAN)
			...
			return buildLollipopNotification(songHelper);
...
/*				newCursor.moveToPosition(j);
    ...
    public boolean toggleShuffleMode() {
    	if (isShuffleOn()) {
			...
			//mApp.getSharedPreferences().edit().putString(Common.LAST_PLAYED_SONG_INDEX, ).commit();
