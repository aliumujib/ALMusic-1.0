 ...
 */
package com.alium.ALMPlayer.MainActivity;
...
import com.alium.ALMPlayer.Drawers.NavigationDrawerFragment;
import com.alium.ALMPlayer.Drawers.QueueDrawerFragment;
import com.alium.ALMPlayer.FoldersFragment.FilesFoldersFragment;
import com.alium.ALMPlayer.GridViewFragment.GridViewFragment;
import com.alium.ALMPlayer.Helpers.UIElementsHelper;
import com.alium.ALMPlayer.ListViewFragment.ListViewFragment;
import com.alium.ALMPlayer.NowPlayingActivity.NowPlayingActivity;
import com.alium.ALMPlayer.R;
import com.alium.ALMPlayer.Services.PinGMusicSongsService;
...
public class MainActivity extends FragmentActivity {
    ...
    private void applyLollipopBars() {

        //Lollipop navigation/status bar.
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_now_playing:
                if (!mApp.isServiceRunning()) {
                    playAll(false);
                    Toast.makeText(mContext, R.string.no_song_playing, Toast.LENGTH_LONG).show();
                    /*Intent intent = new Intent(this, MainActivity.class);
                    mApp.startService(intent);
                    mApp.getService().skipToTrack(//mApp.getSharedPreferences().getInt(Common.LAST_PLAYED_SONG_INDEX, 2)
                    mApp.getCurrentLibrary().length()-1);
                    mApp.getService().setRepeatMode(Common.REPEAT_SONG);
                    startActivity(new Intent(mContext, NowPlayingActivity.class));
                    mApp.getCurrentLibraryIndex();*/
                } else {
                    startActivity(new Intent(mContext, NowPlayingActivity.class));
                    mApp.getCurrentLibraryIndex();
                }
