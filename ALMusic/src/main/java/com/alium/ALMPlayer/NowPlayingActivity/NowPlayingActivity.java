 ...
 */
package com.alium.ALMPlayer.NowPlayingActivity;
...
import com.alium.ALMPlayer.Animations.FadeAnimation;
import com.alium.ALMPlayer.DBHelpers.DBAccessHelper;
import com.alium.ALMPlayer.Drawers.QueueDrawerFragment;
import com.alium.ALMPlayer.Helpers.SongHelper;
...
import com.alium.ALMPlayer.Services.AudioPlaybackService;
import com.alium.ALMPlayer.Utils.Common;
import com.velocity.view.pager.library.VelocityViewPager;
import com.alium.ALMPlayer.Utils.BitmapProcessor;
//import com.alium.ALMPlayer.Utils.ColorArt;
...
public class NowPlayingActivity extends FragmentActivity {
    ...
    //AlbumArt Processors
    BitmapProcessor mBitMapProcessor;
    Bitmap mIcon;
    DBAccessHelper mDBDbAccessHelper;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAudioPlaybackService = mApp.getService();
        //Set the UI theme.
        setTheme();
        ...
        mSeekbar.setProgressDrawable(UIElementsHelper.getProgressDrawable(mContext));
        ...
        mHandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                setCustomControlsBackGround();

                if (!mApp.isServiceRunning()) {
                    setmCurrentSongIndex(mApp.getService().getCurrentSongIndex());
                }
            }
        }, 700);
    ...
    private int getCurrentSongIndex() {
        return mApp.getService().getCurrentSongIndex();
    }

    private void setmCurrentSongIndex(int songIndex) {
        mApp.getSharedPreferences().edit().putInt(Common.LAST_PLAYED_SONG_INDEX, songIndex).commit();
    ...
    private void setCustomControlsBackGround() {
        /*try {
            mIcon = mApp.getService().getCurrentSong().getAlbumArt();
            ColorArt mColorArt = new ColorArt(mIcon);
            int color = mColorArt.getBackgroundColor();
            int bluminance = getLuminance(Color.GRAY);
            int cluminance = getLuminance(color);
            if (cluminance <= 10 || cluminance >= 250) {
                mControlsLayoutHeader.setBackgroundColor(mColorArt.getPrimaryColor());
            } else
                mControlsLayoutHeader.setBackgroundColor(mColorArt.getBackgroundColor());
            Log.d("SUCCESS", "COLOR CODE IS " + mColorArt.getBackgroundColor() + " lumincance is " + getLuminance(color));
        } catch (NullPointerException nullPointerException) {
            mControlsLayoutHeader.setBackgroundResource(UIElementsHelper.getGridViewCardBackground(mContext));
            Log.d("ERROR", "SHIT don't work");
            nullPointerException.printStackTrace();
        }*/

        try {
            mIcon = mApp.getService().getCurrentSong().getAlbumArt();
            Palette palette = Palette.generate(mIcon);
            ...
            mControlsLayoutHeader.setBackgroundColor(vibrantSwatch.getRgb());
        } catch (Exception e) {
            e.printStackTrace();
            Log.d("CONTROLS", "WELL I HAD TO DO SOMETHING");
        }
    }
    ...
    private void setRepeatButtonIcon() {
        if (mApp.isServiceRunning())
            if (mApp.getService().getRepeatMode() == Common.REPEAT_OFF) {
                ...
                mRepeatButton.setImageResource(R.drawable.ic_repeat_24px_blue);
            } else if (mApp.getService().getRepeatMode() == Common.REPEAT_SONG) {
                mRepeatButton.setImageResource(R.drawable.ic_repeat_one_24px_blue);
            } else if (mApp.getService().getRepeatMode() == Common.A_B_REPEAT) {
                mRepeatButton.setImageResource(R.drawable.repeat_green);
            } else
                mRepeatButton.setImageResource(UIElementsHelper.getIcon(mContext, "repeat"));
    ...
    private void setShuffleButtonIcon() {
        if (mApp.isServiceRunning())
            if (mApp.getService().isShuffleOn() == true) {
                mShuffleButton.setImageResource(R.drawable.ic_shuffle_24px_blue);
            } else {
    ...
    private VelocityViewPager.OnPageChangeListener mPageChangeListener = new VelocityViewPager.OnPageChangeListener() {
        ...
        public void onPageSelected(int newPosition) {
            ...
            try {
                //probably fixed that bug that the colu=our doesn't cahnge after a long scroll, not sure sha
                mIcon = mApp.getService().getCurrentSong().getAlbumArt();
                Palette palette = Palette.generate(mIcon);
                Palette.Swatch vibrantSwatch = palette.getVibrantSwatch();
                Palette.Swatch vibrantLightSwatch = palette.getLightVibrantSwatch();
    ...
    public void onResume() {
        ...
        try {
            ...
            Log.d("Onresume:", "Excreptoion from kickstarter");
        }
